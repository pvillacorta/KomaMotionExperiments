...
using Plots
# Mx
px = plot(;legend=:outertopright, ylabel="Mx", yrange=(-1.2, 0.9))
for (i, v) in enumerate(vs)
    x, y, z = get_spin_coords(objs[i].motion, objs[i].x, objs[i].y, objs[i].z, dur(seq))
    plot!(z * 1e3, real.(Ms[i].xy), label="v = $(v * 1e2) cm/2")
end
# My
py = plot(;legend=:outertopright, ylabel="My", yrange=(-1.0, 1.0))
for (i, v) in enumerate(vs)
    x, y, z = get_spin_coords(objs[i].motion, objs[i].x, objs[i].y, objs[i].z, dur(seq))
    plot!(z * 1e3, imag.(Ms[i].xy), label="v = $(v * 1e2) cm/2")
end
# My
pz = plot(;legend=:outertopright, xlabel="z (mm)", ylabel="Mz", yrange=(-0.2, 1.2))
for (i, v) in enumerate(vs)
    x, y, z = get_spin_coords(objs[i].motion, objs[i].x, objs[i].y, objs[i].z, dur(seq))
    plot!(z * 1e3, Ms[i].z, label="v = $(v * 1e2) cm/2")
end
plot(px, py, pz, layout=(3,1), size=(390, 700))
